...
import { cloudinary, upload } from "../config/cloudinary.js"; // KEEP FOR REFERENCE

// @desc    Create FEO (NOW WITH BASE64 SUPPORT)
// @route   POST /api/feo
// @access  Private/Admin
export const createFEO = asyncHandler(async (req, res) => {
  console.log("=== Create FEO Request (BASE64) ===");
  ...
  const {
    ...
    officeContact,
    password,
    profileImageBase64, // NEW: Accept Base64
  } = req.body;
  ...
  // NEW: Handle Base64 profile image
  if (profileImageBase64) {
    console.log("📷 Processing Base64 image...");
    console.log("Base64 length:", profileImageBase64.length);
    ...
    console.log("✅ Base64 image saved");
  }
...
// OLD: Cloudinary version (KEPT FOR REFERENCE)
export const createFEOCloudinary = asyncHandler(async (req, res) => {
  console.log("=== Create FEO Request (CLOUDINARY) ===");
...
// @desc    Update FEO (NOW WITH BASE64 SUPPORT)
// @route   PUT /api/feo/:id
// @access  Private/Admin
export const updateFEO = asyncHandler(async (req, res) => {
  console.log("=== Update FEO Request (BASE64) ===");
  console.log("FEO ID:", req.params.id);
  console.log("Body keys:", Object.keys(req.body));
...
// OLD: Cloudinary version (KEPT FOR REFERENCE)
export const updateFEOCloudinary = asyncHandler(async (req, res) => {
  const feo = await FEO.findById(req.params.id);
...
export const deleteFEO = asyncHandler(async (req, res) => {
  ...
  // Delete profile image from cloudinary if exists (CLOUDINARY VERSION)
  if (feo.profileImage?.publicId) {
    try {
      await cloudinary.uploader.destroy(feo.profileImage.publicId);
...
// @desc    Update FEO profile (self) - NOW WITH BASE64 SUPPORT
// @route   PUT /api/feo/profile/me
// @access  Private (FEO)
export const updateFEOProfile = asyncHandler(async (req, res) => {
  console.log("=== Update FEO Profile Request (BASE64) ===");
  console.log("Body keys:", Object.keys(req.body));
...
// OLD: Cloudinary version (KEPT FOR REFERENCE)
export const updateFEOProfileCloudinary = asyncHandler(async (req, res) => {
  const feo = await FEO.findById(req.user._id);
