...
export const generatePDFReport = async (res, title, data) => {
  try {
    ...
    doc.moveDown(1.2);
    doc.moveTo(50, doc.y).lineTo(550, doc.y).stroke("#19A7CE");
    doc.moveDown(1);
    ...
    data.forEach((s, index) => {
      ...
      doc
        ...
        .fontSize(14)
        .text(
          `${index + 1}. ${s.CommonName || "Unknown Species"}`,
          textX,
          yTop
        );
      doc.moveDown(0.3);
      ...
      if (s.ImageURL) {
        try {
          ...
          doc
            .image(imagePath, 420, yTop, { width: 80, height: 80 })
            .rect(420, yTop, 80, 80)
            .stroke();
        } catch {
    ...
    for (let i = 0; i < pageCount; i++) {
      ...
      doc.text(`Page ${i + 1} of ${pageCount}`, -50, bottom, {
        align: "right",
      });
    }
...
export const getEndangeredReport = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error generating endangered report", error });
  }
};
...
export const getVulnerableReport = async (req, res) => {
  try {
    ...
    await generatePDFReport(
      res,
      "Vulnerable Species Report",
      vulnerableSpecies
    );
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error generating vulnerable report", error });
  }
};
//--
...
export const addSpecies = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Species added successfully", data: newSpecies });
  } catch (error) {
...
export const deleteSpecies = async (req, res) => {
  try {
    ...
    // Record history
    await SpeciesHistory.create({
      action: `Deleted species - ${
        deletedSpecies.CommonName || deletedSpecies.ScientificName
      }`,
      speciesId: id,
...
export const getSpeciesDashboard = async (req, res) => {
  try {
    ...
    const recentCount = speciesList.filter(
    ...
    speciesList.forEach((species) => {
      const month = new Date(species.createdAt).toLocaleString("default", {
        month: "short",
      });
...
export const getWikipediaInfo = async (req, res) => {
  try {
    ...
    if (error.response && error.response.status === 404) {
      ...
      return res
        .status(404)
        .json({ message: "No information found on Wikipedia" });
    } else {
